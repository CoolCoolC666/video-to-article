...
def build_blog_prompt_input(
    ...
    2026-09-07 增强：local 平台也从路径提取 UP 主 / BV 号 / 原始标题前缀，
    帮 LLM 识别游戏类型（避免"崩坏星穹铁道错猜成 LOL"）。
    """
    ...
    if source_type == "youtube":
        ...
        if local_path:
            ...
            p = _P(local_path)
            # 路径片段 "...Bilibili-甘泽成谣雨成诗\audio\xxx.mp3" → UP="甘泽成谣雨成诗"
            for part in p.parts:
                if part.startswith("Bilibili-"):
                    up = part.replace("Bilibili-", "", 1)
                    if up:
                        lines.append(f"UP 主（从路径推断）：{up}")
                    break
            # 文件名里 BV 号
            import re as _re
            m = _re.search(r"(BV[0-9A-Za-z]+)", p.stem)
            if m:
                lines.append(f"视频 BV 号：{m.group(1)}")
    else:
