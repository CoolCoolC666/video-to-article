...
默认仍只下载音频用于转写；加 --save-video 可额外保留视频；--download-only 仅下载不转写。
    ...
    # 2026-09-09：qwen_asr 引擎的 device 覆盖（GPU 1 掉驱动时用 --qwen-device cpu）
    parser.add_argument(
        "--qwen-device",
        type=str,
        default=None,
        choices=["auto", "cuda", "cpu"],
        help="qwen_asr 引擎 device 覆盖：auto=自动选/降级，cuda=强制 GPU，"
        "cpu=强制 CPU（适合 GPU 驱动掉了）。未指定时读 config.transcribe.qwen_asr.device",
    )
    parser.add_argument("--list-prompts", action="store_true", help="列出所有可用的提示词")
    return parser


def resolve_asr_engine(args) -> str:
    """Resolve ASR engine: explicit --asr-engine wins; else read config.transcribe.asr_engine; fallback to 'funasr'."""
    explicit = getattr(args, "asr_engine", None)
    if explicit:
        return explicit
    try:
        config = load_config()
        ...
        config = {}
    return config.get("transcribe", {}).get("asr_engine", "funasr") or "funasr"
...
def main() -> None:
    ...
    # Resolve --asr-engine from config when not provided on the CLI
    resolved_asr = resolve_asr_engine(args)
    if not getattr(args, "asr_engine", None):
        args.asr_engine = resolved_asr
        print(f"ASR 引擎: {resolved_asr} (来自 config.transcribe.asr_engine)")

    # 2026-09-09: --qwen-device CLI 覆盖 → 写回全局 config["transcribe"]["qwen_asr"]["device"]
    # 这样下游 processor / qwen_asr.py 不用改任何调用链路，统一从 config dict 读
    if getattr(args, "qwen_device", None):
        try:
            cfg = load_config() or {}
            tr = cfg.setdefault("transcribe", {})
            qa = tr.setdefault("qwen_asr", {})
            qa["device"] = args.qwen_device
            save_config(cfg)
            print(f"Qwen3-ASR device 覆盖: {args.qwen_device} (已写回 config.json)")
        except Exception as e:
            print(f"⚠ --qwen-device 写回 config 失败: {e}（将走 config 默认值）")
