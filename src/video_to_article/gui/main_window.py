...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.release_model_btn = PushButton("释放 ASR 模型")
        self.release_model_btn.setToolTip(
            "2026-09-10: 手动释放 Qwen3-ASR 模型占用的 GPU 显存。\n"
            "让出显存给游戏/OCR/本地 LLM；下次转写会重新加载（约 5 秒）。\n"
            "模型空闲时也可让游戏独享全部 12GB。"
        )
        self.release_model_btn.clicked.connect(self._release_qwen_asr_model)
        self.cleanup_chunks_btn = PushButton("清理临时缓存")
        self.cleanup_chunks_btn.setToolTip(
            "2026-09-10: 清残留 qwen_asr_chunks_* 切段 tempdir（%TEMP% 下）。\n"
            "强杀/断电后 finally 没跑时手动清；不重启 GUI。\n"
            "下次转写会自动重建。"
        )
        self.cleanup_chunks_btn.clicked.connect(self._cleanup_temp_chunks)
        self.copy_cli_btn = PushButton("复制命令行")
        self.copy_cli_btn.clicked.connect(self.copy_equivalent_cli)

        self.asr_status_label = QLabel("ASR: 未加载")
        self.asr_status_label.setToolTip("Qwen3-ASR 模型当前是否在 GPU 显存里（5 秒刷新）")
        self.asr_status_label.setMinimumWidth(180)
        self.asr_status_label.setStyleSheet("color: #888; padding: 0 8px;")
        ...
        # 2026-09-10: ASR 模型状态定时刷新（5 秒）— 让用户知道 GPU 显存占用
        self._asr_status_timer = QTimer(self)
        self._asr_status_timer.timeout.connect(self._refresh_asr_status)
        self._asr_status_timer.start(5000)
    ...
    def _refresh_asr_status(self) -> None:
        """2026-09-10: 每 5 秒刷一次 Qwen3-ASR 模型状态（已加载/未加载 + model_id）"""
        try:
            from .media.qwen_asr import get_cached_model_id
            mid = get_cached_model_id()
        except Exception:
            mid = None
        if mid:
            # 截断 model_id 避免 label 太长
            short = mid.split("/")[-1] if "/" in mid else mid
            self.asr_status_label.setText(f"ASR: {short} (显存)")
            self.asr_status_label.setStyleSheet("color: #0078D4; padding: 0 8px; font-weight: 600;")
        else:
            self.asr_status_label.setText("ASR: 未加载")
            self.asr_status_label.setStyleSheet("color: #888; padding: 0 8px;")

    def _release_qwen_asr_model(self) -> None:
        """2026-09-10: 手动释放 Qwen3-ASR 模型占用的 GPU 显存

        场景：用户在跑 OCR/本地 LLM/游戏时，主动让出 GPU 显存
        代价：下次转写会重新加载 1.7B 模型（~5 秒）
        """
        try:
            from .media.qwen_asr import _release_cached_model
            _release_cached_model()
            self._refresh_asr_status()
            self.log_panel.append(
                "\n[GUI] 已手动释放 Qwen3-ASR 模型（GPU 显存已让出，下次转写会重新加载 5s）\n"
            )
            self.statusBar().showMessage("ASR 模型已释放", 3000)
        except Exception as exc:
            QMessageBox.warning(self, "释放失败", str(exc))

    def _cleanup_temp_chunks(self) -> None:
        """2026-09-10: 手动清理 qwen_asr_chunks_* 残留 tempdir

        场景：用户强杀进程/断电后 finally 没跑；或者想主动清缓存腾出 C 盘
        """
        import glob
        import shutil
        import tempfile
        ...
        msg = f"\n[GUI] 清理完成：{cleaned} 个 tempdir，腾出 {mb_freed:.1f} MB\n"
        self.log_panel.append(msg)
        self.statusBar().showMessage(
            f"已清理 {cleaned} 个切段目录，腾出 {mb_freed:.1f} MB", 4000
        )
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        if self._worker is not None:
            ...
            if reply != QMessageBox.Yes:
                ...
                return
            self.stop_job()
            # 2026-09-10: 强等 worker 结束（避免关闭后文件仍被占用）
            if self._thread is not None:
                self._thread.quit()
                if not self._thread.wait(10000):
                    self.log_panel.append(
                        "\n[GUI] worker 10s 未结束，强制 terminate\n"
                    )
                    self._thread.terminate()
                    self._thread.wait(2000)
        # 2026-09-10: 关闭 GUI 立刻释放 ASR 模型（让出显存，不用等 Python 进程死）
        try:
            from .media.qwen_asr import _release_cached_model
            _release_cached_model()
        except Exception:
            pass
        # 2026-09-10: 关闭时也清残留切段 tempdir（保险，atexit 也会兜底）
        try:
            import glob
            import shutil
            import tempfile
