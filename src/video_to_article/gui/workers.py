...
class JobWorker(QObject):
    ...
    def __init__(self, request: JobRequest) -> None:
        ...
        self._cancel = False
        # 2026-09-08: 取消事件 (threading.Event 让 process_batch 检查)
        import threading
        self._cancel_event = threading.Event()

    def request_cancel(self) -> None:
        self._cancel = True
        # 2026-09-08: 同时设 Event，process_batch 主循环会查
        try:
    ...
    def _run_batch(self) -> Any:
        ...
        # 2026-09-08: 进度回调 (Qt signal emit) + 取消事件 (用户点停止时设)
        def _cb(i: int, total: int, status: str, url: str) -> None:
            try:
                self.progress.emit(i, total)
                if status == "success":
                    self.stage.emit(f"✓ {i}/{total}: {url}")
                elif status == "failed":
                    self.stage.emit(f"✗ {i}/{total}: {url}")
            except Exception:
                pass
