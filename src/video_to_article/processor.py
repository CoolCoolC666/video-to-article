...
def process_batch(
    ...
    repair_delay: int = 0,
    save_video: bool = False,
    # 2026-09-08: GUI 进度回调 + 取消事件
    # progress_callback: Optional[Callable[[int, int, str, str], None]] = None
    #   每完成一个 video 调一次，参数 (i, total, status, url)
    #   status: "success" / "failed" / "skipped"
    # cancel_event: Optional[threading.Event] = None
    #   设了 cancel_event.set() 后，当前 video 转写完后会退出循环
    progress_callback=None,  # type: ignore[assignment]
    cancel_event=None,  # type: ignore[assignment]
) -> List[dict]:
    ...
    # 2026-09-08: 进度条 - _total_start 记录批次开始时间
    results = []
    ...
    for i, url in enumerate(video_urls, 1):
        ...
        try:
            ...
            # 2026-09-08: 错误分类 + 友好提示，避免 CUDA traceback 刷屏
            err_str = str(e)
            err_lower = err_str.lower()
            if "out of memory" in err_lower or "cuda error" in err_lower:
                friendly = "CUDA OOM (GPU 显存不足)"
                hint = "建议: nvidia-smi 查残留 Python 进程 + 关 YilanChengWen 0.4.5 GUI"
            elif "auth_or_bot_check" in err_lower or "412" in err_str or "403" in err_str:
                friendly = "B 站风控 (cookies 过期/缺失)"
                hint = "建议: 更新 data/cookies/Bilibili.txt"
            elif "no_caption_tracks" in err_lower:
                friendly = "B 站无字幕 (已回退 ASR)"
                hint = ""
            elif "modulenotfounderror" in err_lower or "missing dependency" in err_lower:
                friendly = "Python 依赖缺失"
                hint = "建议: .venv/Scripts/pip install -r requirements.txt"
            else:
                friendly = err_str[:200].replace("\n", " ")
                hint = ""
            logger.error(f"  原因: {friendly}")
            if hint:
            ...
            results.append({
                ...
                "error": friendly,
                "error_hint": hint,
                "raw_error_snippet": err_str[:500],  # 留 500 字符供调试
            })
        finally:
            # 2026-09-08: 进度条 - 无论成功失败都打 (ETA + 平均)
            now = time.time()
            total_elapsed = now - _total_start
            avg = total_elapsed / i
            eta_sec = avg * (len(video_urls) - i)
            eta_str = time.strftime('%H:%M:%S', time.localtime(now + eta_sec))
            is_success = bool(results[-1].get("success")) if results else False
            status_char = "✓" if is_success else "✗"
            print(
                f"  {status_char} [{i}/{len(video_urls)}] "
                f"已用 {total_elapsed/60:.1f}min | 平均 {avg:.1f}s/视频 | "
                f"预计剩余 {eta_sec/60:.1f}min (完 ETA {eta_str})",
                flush=True,
            )
            # 2026-09-08: GUI 进度回调（供 PyQt QThread 同步进度条用）
            if progress_callback is not None:
                try:
                    status = "success" if is_success else "failed"
                    progress_callback(i, len(video_urls), status, url)
                except Exception as cb_err:
                    logger.warning(f"progress_callback 调用失败: {cb_err}")
            # 2026-09-08: 取消检查（GUI 点停止时 cancel_event.set()）
            if cancel_event is not None and cancel_event.is_set():
                logger.warning(f"用户取消，已处理 {i}/{len(video_urls)} 个视频")
                break
    ...
    # 2026-09-08: 报告清理 - 保留最近 5 份，更早自动归档到 _archive/
    report_dir = batch_output_dir / "_batch_reports"
    report_dir.mkdir(parents=True, exist_ok=True)
    KEEP_REPORTS = 5
    try:
        existing = sorted(
            report_dir.glob("batch_report_*.json"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )
        if len(existing) >= KEEP_REPORTS:
            archive_dir = report_dir / "_archive"
            archive_dir.mkdir(exist_ok=True)
            for old in existing[KEEP_REPORTS - 1:]:
                try:
                    import shutil
                    shutil.move(str(old), str(archive_dir / old.name))
                    logger.info(f"  归档旧报告: {old.name} → _archive/")
                except OSError as move_err:
                    logger.warning(f"  归档失败: {old.name} ({move_err})")
    except Exception as cleanup_err:
        logger.warning(f"报告清理失败（不影响生成）: {cleanup_err}")
    ...
    print(f"\n详细报告已保存: {report_file}")
    # 2026-09-08: try/finally 释放 Qwen3-ASR cache
    # 不管正常完成还是异常逃出（Ctrl-C / 报告写入失败）都释放
    try:
        return results
    finally:
        try:
            ...
            logger.warning(f"释放 Qwen3-ASR cache 失败（不影响结果）: {release_err}")
